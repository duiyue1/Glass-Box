...
- `GB_MEM_MAX_L0` / `GB_MEM_MAX_ATOMS` — 记忆规模上限（默认 500 / 2000）。见下面「记忆」一节
...
分层记忆（L0 原始对话存证 / L1 蒸馏原子 / 按预算检索注入）里有三件事值得说。

**新结论必须赢，旧结论要留痕。** 原先只有「文本完全相同」才算重复，于是"项目用 npm"和
"项目改用 pnpm"会并存，而排序是词命中优先、时间次之——**旧那条完全可能排在前面**，
agent 就拿着废弃的约定干活。这是正确性问题，不是浪费问题。现在两条规则处理它，都不调模型
（记忆要在零凭证下也能跑）：近重复合并（同类、2-gram Jaccard ≥ 0.7），以及用户明说
「改用 / 不再 / 换成」时让同类旧结论让位。被推翻的**不删**，只是不再注入——
agent 记错事情时，第一个要回答的问题是"它为什么以为是这样"，悄悄删掉就永远查不出来了。
配合 `memory.superseded` 事件，能还原出"什么时候、因为哪句话、按哪种判定被推翻的"。

能力边界写在测试里，而不是靠人记得：2-gram 相似度只认**改动很小的同一句话**，认不出改写
（"偏好中文回答" vs "希望用中文回复" 只有 0.25，不会合并）。想合并改写得上模型，那会让记忆依赖凭证。

**和分叉的交互是被已有测试抓出来的。** 如果"推翻者"本身产生在分叉丢弃的那段时间里，
那么在当前时间线上这次推翻**根本没发生过**，被盖掉的旧结论必须复活——否则分叉回到过去，
却丢了那时明明还成立的事实。

**打分补齐成真的 BM25。** 原来是 `terms.filter((t) => text.includes(t)).length`——数命中了几个词，
注释却自称"BM25 的极简版"。缺的三样各有代价：没有 idf，让「项目」这种到处都有的词和专名等权；
没有 tf，让反复在讲某事的记忆和只提过一次的同分；没有长度归一，让长条目永远占优。
现在 tf / idf / 长度归一都在（k1=1.2、b=0.75），顺带修掉英文 `includes` 的子串误命中（`npm` 不再命中 `npmrc`）。

**规模有上限了。** 原来纯追加。几十条无所谓，几千条时三头受损：落盘无限膨胀、每次检索线性扫全量、
**噪音条目变多把好条目挤出注入预算**。现在 L0 滚动保留（它是存证、不是检索源，丢了不影响召回），
L1 按「已被推翻 → 从没命中过 → 最久没命中」的顺序淘汰，并发 `memory.pruned`——
静默淘汰的症状是"agent 莫名忘了以前说过的话"，那是最难查的一类问题。
